...
function PokeCard({ singlePokemon }) {
  ...
  useEffect(() => {
    ...
    fetchPokemonDetails(singlePokemon.url);
  }, [singlePokemon.url]);

    useEffect(() => {
      // Fetch type sprites for all types
        ...
        if (details.types && details.types.length > 0) {
          const typePromises = details.types.map(async (typeInfo) => {
            try {
              ...
              return {
                name: typeInfo.type.name, // Type name
                sprite: typeData.sprites?.["generation-viii"]?.["sword-shield"], // Sprite URL
              };
            } catch (error) {
              console.error(
                `Error fetching type details for ${typeInfo.type.name}:`,
                error
              );
              return { name: typeInfo.type.name, sprite: null }; // Fallback
            }
          });

          // Wait for all type data to load
          const fetchedTypes = await Promise.all(typePromises);
          setTypesWithSprites(fetchedTypes.filter(Boolean)); // Filter out failed fetches
        }
      };
...
<div className="mb-3">#{details?.id ? details.id.toString().padStart(4, '0') : "#----"}</div>
        ...
        <div className="flex flex-col items-center sm:items-start ">
          ...
          {/* Render All Types with Sprites or Placeholder */}
